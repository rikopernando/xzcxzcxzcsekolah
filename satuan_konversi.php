<?php include 'session_login.php';
 ...
 $satuan_konversi = $db->query("SELECT sk.id, sk.id_satuan, sk.id_produk, sk.konversi, sk.harga_pokok, s.nama FROM satuan_konversi sk INNER JOIN satuan s ON sk.id_satuan = s.id WHERE sk.id_produk = '$id_produk'");
...
<table id="tableuser" class="table table-bordered">
    ...
    while ($data = mysqli_fetch_array($satuan_konversi))
    {
      ...
      echo " <td class='edit-konversi' data-id='".$data['id']."'><span id='text-konversi-".$data['id']."'>". rp($data['konversi']) ."</span> <input type='hidden' id='input-konversi-".$data['id']."' value='".$data['konversi']."' class='input_konversi' data-id='".$data['id']."' autofocus=''> </td>
      ...
      <td> <button class='btn btn-danger btn-hapus' data-id='". $data['id'] ."' data-satuan='". $data['id_satuan'] ."'> <i class='fa fa-trash'> </i> Hapus </button> </td>
...
//fungsi untuk menambahkan data
    ...
    $.post('proses_tambah_satuan_konversi.php',{nama_satuan_konversi:nama_satuan_konversi,konversi:konversi,harga_pokok:harga_pokok,id_produk:id_produk,kode_produk:kode_produk},function(data){
